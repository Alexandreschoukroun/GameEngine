...
- **M4.5 — Animation squelettique** : chargement des squelettes et des clips depuis glTF (cgltf sait déjà les lire, aucune dépendance nouvelle), skinning sur GPU, lecteur d'animation avec interpolation des poses et transitions simples entre états. *Livrable : un personnage qui marche en boucle et change d'animation.* **Périmètre strict** : pas d'animation faciale, pas de ragdoll, pas de cinématique inverse, pas d'arbre de mélange élaboré — une seule animation active à la fois, avec fondu entre deux. *Placé ici et pas ailleurs : l'antagoniste de M7 doit pouvoir marcher, et un monstre non animé qui glisse sur le sol produit l'effet inverse de celui recherché. Ce jalon manquait à la version initiale du document.*
